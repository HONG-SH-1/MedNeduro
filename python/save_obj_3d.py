...
def get_brain_mask(img3d):
    ...
    """
        ...
        binary_closing (닫기) :
            동작 : 이미지를 부풀리기(Dilation) -> 다시 깎기(Erosion)
            효과 : 다시 2mm만큼 겉면을 깎아내어 원래 크기로 되돌립니다. 겉모양은 원래대로 돌아왔지만, 
                    안에서 이미 메워진 구멍들은 그대로 단단하게 채워져 있습니다.
    """
    ...
    """
        cc_label(mask) :
            서로 붙어있는 픽셀들끼리 같은 번호를 매깁니다. (예 : 뇌는 1번, 눈알은 2번, 두개골 조각은 3번..)
        regionprops(lab) : 
